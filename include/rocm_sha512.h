...
#ifndef INCLUDE_MMX_ROCM_SHA512_H_
#define INCLUDE_MMX_ROCM_SHA512_H_
...
__device__ inline
void rocm_sha512_chunk(const uint64_t* msg, uint64_t* state)
{
	...
	// on AMD and silently corrupts the final 192-byte hash used by F1.
	uint64_t w[16];

	for(int i = 0; i < 16; ++i) {
		w[i] = rocm_bswap_64(msg[i]);
	}
...
#pragma unroll
	...
	{
		if(i >= 16) {
			const uint64_t s0 = sha512_s0(w[(i - 15) & 15]);
			const uint64_t s1 = sha512_s1(w[(i - 2) & 15]);
 ...
 *
 * Callers often fill a uint32_t[] then cast to uint64_t*. On HIP/clang -O2 that
 * type-pun can make prior stores invisible (silent zero message). The memory
 * clobber below forces those stores to complete before we read msg.
 */
__device__ inline
void rocm_sha512(uint64_t* msg, const uint32_t length, uint64_t* hash)
{
	...
	msg[num_chunks * 16 - 1] = rocm_bswap_64(num_bits);

	for(int i = 0; i < 8; ++i) {
		hash[i] = SHA512_INIT[i];
	}
	// Do not #pragma unroll — num_chunks is runtime-dependent (1 or 2 for F1).
	for(uint32_t i = 0; i < num_chunks; ++i) {
		rocm_sha512_chunk(msg + i * 16, hash);
	}
	// Ensure hash written as uint64 is visible when read as uint32 by callers.
	asm volatile("" : : "r"(hash) : "memory");
	for(int i = 0; i < 8; ++i) {
		hash[i] = rocm_bswap_64(hash[i]);
	}
	asm volatile("" : : "r"(hash) : "memory");
}
...
#endif /* INCLUDE_MMX_ROCM_SHA512_H_ */
