...
#ifndef INCLUDE_MMX_ROCM_SHA256_H_
#define INCLUDE_MMX_ROCM_SHA256_H_
...
#define SHA256_ZL25(n) ((rocm_rotl_32((n), 25) ^ rocm_rotl_32((n), 14) ^ ((n) >> 3U)))
#define SHA256_ZL15(n) ((rocm_rotl_32((n), 15) ^ rocm_rotl_32((n), 13) ^ ((n) >> 10U)))
#define SHA256_ZL26(n) ((rocm_rotl_32((n), 26) ^ rocm_rotl_32((n), 21) ^ rocm_rotl_32((n), 7)))
#define SHA256_ZL30(n) ((rocm_rotl_32((n), 30) ^ rocm_rotl_32((n), 19) ^ rocm_rotl_32((n), 10)))
...
__device__ inline
void rocm_sha256_chunk(const uint32_t* msg, uint32_t* state)
{
	uint32_t w[64];

	for(int i = 0; i < 16; ++i) {
		w[i] = rocm_bswap_32(msg[i]);
	}
 ...
 */
__device__ inline
void rocm_sha256(uint32_t* msg, const uint32_t length, uint32_t* hash)
{
	...
	msg[num_chunks * 16 - 1] = rocm_bswap_32(num_bits);

	for(int i = 0; i < 8; ++i) {
		hash[i] = SHA256_INIT[i];
	}
	for(uint32_t i = 0; i < num_chunks; ++i) {
		rocm_sha256_chunk(msg + i * 16, hash);
	}
	for(int i = 0; i < 8; ++i) {
		hash[i] = rocm_bswap_32(hash[i]);
	}
}
...
#endif /* INCLUDE_MMX_ROCM_SHA256_H_ */
