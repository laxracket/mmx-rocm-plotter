...
#ifndef INCLUDE_MMX_ROCM_ENCODING_H_
#define INCLUDE_MMX_ROCM_ENCODING_H_
...
__device__ inline
uint64_t write_bits(uint64_gpu* dst, uint64_t value, const uint64_t bit_offset, const uint32_t num_bits)
{
	if(num_bits < 64) {
...
__device__ inline
uint64_t atomic_write_bits(uint64_gpu* dst, uint64_t value, const uint64_t bit_offset, const uint32_t num_bits)
{
	if(num_bits < 64) {
		value &= ((uint64_t(1) << num_bits) - 1);
	}
...
__device__ inline
uint64_t read_bits(const uint64_gpu* src, const uint64_t bit_offset, const uint32_t num_bits)
{
	uint32_t count = 0;
	...
	while(count < num_bits) {
		...
		const uint32_t bits = std::min(num_bits - count, 64 - shift);
...
#endif /* INCLUDE_MMX_ROCM_ENCODING_H_ */
