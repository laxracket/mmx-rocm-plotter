...
public:
	...
	virtual size_t upload(void* dst, hipStream_t stream, const size_t count, const size_t src_offset = 0) = 0;

	virtual void copy(const void* src, const size_t dst_offset, const size_t count) = 0;

	template<typename T>
	size_t upload(Buffer<T>& dst, hipStream_t stream)
	{
		return upload(dst.data(), stream, size());
	}

	template<typename T>
	size_t upload(Buffer<T>& dst, hipStream_t stream, const size_t count, const size_t src_offset = 0, const size_t dst_offset = 0)
	{
...
public:
	...
	// NEEDS TO BE THREAD SAFE
	size_t upload(void* dst, hipStream_t stream, const size_t count, const size_t src_offset_ = 0) override
	{
		...
		for(size_t i = src_offset / BUCKET_CHUNK_SIZE; dst_offset < count && i < chunks.size(); ++i)
		{
			...
			if(auto chunk = chunks[i]) {
				...
				hip_check(hipMemcpyAsync(p_dst + dst_offset, chunk->data(off), len, hipMemcpyHostToDevice, stream));
			} else {
				hip_check(hipMemsetAsync(p_dst + dst_offset, 0, len, stream));
			}
			dst_offset += len;
			src_offset += len;
		}
