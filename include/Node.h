...
private:
	...
	void upload_buffer(Buffer<T>& dst, const Buffer<S>& src, hipStream_t stream, const size_t count, const size_t offset = 0);

	template<typename T>
	void download_buffer(Buffer<T>& dst, const Buffer<T>& src, const size_t num_bytes, hipStream_t stream);

	template<typename T>
	void download_buffer(Buffer<T>& dst, const Buffer<T>& src, hipStream_t stream);
...
private:
	...
	std::vector<hipStream_t> copy_stream;

	std::vector<hipEvent_t> sync_event;
	std::vector<hipEvent_t> sync_event2;
	std::vector<hipEvent_t> upload_event;
	std::vector<hipEvent_t> download_event;
...
template<typename T, typename S>
void Node::upload_buffer(Buffer<T>& dst, const Buffer<S>& src, hipStream_t stream, const size_t count, const size_t offset)
{
	if(src.size() < offset + count) {
		throw std::logic_error("src.size() < offset + count");
	}
	...
	hip_check(hipMemcpyAsync(dst.data(), src.data(offset), count * sizeof(T), hipMemcpyHostToDevice, stream));
	g_upload_bytes += count * sizeof(T);
}

template<typename T>
void Node::download_buffer(Buffer<T>& dst, const Buffer<T>& src, const size_t num_bytes, hipStream_t stream)
{
	if(dst.num_bytes() < num_bytes) {
		throw std::logic_error("dst.num_bytes() < num_bytes");
	}
	hip_check(hipMemcpyAsync(dst.data(), src.data(), num_bytes, hipMemcpyDeviceToHost, stream));
	g_download_bytes += num_bytes;
}

template<typename T>
void Node::download_buffer(Buffer<T>& dst, const Buffer<T>& src, hipStream_t stream)
{
