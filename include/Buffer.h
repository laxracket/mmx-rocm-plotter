...
public:
			...
			try {
				switch(type) {
					...
					case MEM_TYPE_DEVICE:
						hip_check(hipMalloc((void**)&data_, num_bytes()));
						break;
					case MEM_TYPE_MANAGED:
						hip_check(hipMallocManaged((void**)&data_, num_bytes()));
						break;
					default:
	...
	void hip_register() {
		if(!is_registered_) {
			hip_check(hipHostRegister(data_, num_bytes(), 0));
			is_registered_ = true;
		}
	}
	...
	void hip_unregister() {
		if(is_registered_) {
			...
			hip_check(hipHostUnregister(data_));
		}
	}
	...
	void upload(const T* host) {
		hip_check(hipMemcpy(data_, host, num_bytes(), hipMemcpyHostToDevice));
	}

	void download(T* host) const {
		hip_check(hipMemcpy(host, data_, num_bytes(), hipMemcpyDeviceToHost));
	}
	...
	void copy_from(const Buffer<T>& src) const
	{
		...
		hip_check(hipMemcpy(data_, src.data(), num_bytes(), kind));
	}

	void memset(int value) {
		hip_check(hipMemset(data_, value, num_bytes()));
	}
	...
	void memset_async(int value, hipStream_t stream) {
		hip_check(hipMemsetAsync(data_, value, num_bytes(), stream));
	}

	void memset_async(int value, uint64_t offset, uint64_t length, hipStream_t stream) {
		hip_check(hipMemsetAsync(data_ + offset, value, length * sizeof(T), stream));
	}

	void advise_read_mostly()
	{
		if(type_ == MEM_TYPE_MANAGED) {
			hip_check(hipMemAdvise(data_, num_bytes(), hipMemAdviseSetReadMostly, -1));
		}
	}
	...
	void advise_location(int device, uint64_t length = 0, uint64_t offset = 0)
	{
		if(type_ == MEM_TYPE_MANAGED) {
			...
			int value = 0;
			hip_check(hipDeviceGetAttribute(&value, hipDeviceAttributeConcurrentManagedAccess, device));
			if(value) {
				hip_check(hipMemAdvise(data_ + offset, length * sizeof(T), hipMemAdviseSetPreferredLocation, device));
			}
		}
	}
...
template<typename T>
void hip_memset(Buffer<T>& buffer, const T& value, hipStream_t stream) {
	hip_memset<T>(buffer.data(), value, buffer.size(), stream);
}

template<typename T>
void hip_memset(Buffer<T>& buffer, const T& value, hipStream_t stream, const size_t count) {
	hip_memset<T>(buffer.data(), value, count, stream);
}

template<typename T>
void hip_memcpy(T* dst, Buffer<T>& src, hipStream_t stream) {
	hip_memcpy<T>(dst, src.data(), src.size(), stream);
}

template<typename T>
void hip_memcpy(T* dst, Buffer<T>& src, hipStream_t stream, const size_t count) {
	hip_memcpy<T>(dst, src.data(), count, stream);
}
