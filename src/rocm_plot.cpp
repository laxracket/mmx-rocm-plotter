...
int _main(int argc, char** argv)
{
	...
	cxxopts::Options options("mmx_rocm_plot",
		"MMX k" + std::to_string(KSIZE) + " ROCm plotter" +
	...
	options.allow_unrecognised_options().add_options()(
		...
		"g, device", "ROCm device (default = 0)", cxxopts::value<int>(device))(
		"r, ndevices", "Number of ROCm devices (default = 1)", cxxopts::value<int>(num_devices))(
...
//		"D, directio", "Use direct IO for final copy (default = false, Linux only)", cxxopts::value<bool>(use_direct_io))(
	...
	for(const auto& final_dir : final_out)
	{
		...
		if(final_dir[0] != '@')
		{
			...
			{
				const std::string path = final_dir + ".rocm_plot_final";
	...
	for(const auto& dir : tmp_dir) {
		const std::string path = dir + ".rocm_plot_tmp";
		if(auto file = fopen(path.c_str(), "wb")) {
			fclose(file);
			remove(path.c_str());
		} else {
	...
	if(tmp_dir2 != "@RAM") {
		const std::string path = tmp_dir2 + ".rocm_plot_tmp";
		if(auto file = fopen(path.c_str(), "wb")) {
			fclose(file);
			remove(path.c_str());
		} else {
	...
	if(tmp_dir3 != "@RAM") {
		const std::string path = tmp_dir3 + ".rocm_plot_tmp";
		if(auto file = fopen(path.c_str(), "wb")) {
			fclose(file);
			remove(path.c_str());
		} else {
	...
	std::cout << "MMX k" + std::to_string(KSIZE) + " ROCm plotter";
	#ifdef GIT_COMMIT_HASH
